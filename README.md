...
- Why `abstract InputToken` with `DigitToken` and `ControlKeyToken` as children?
  ...
  ```
  It allows us have contexts where both `Digit`s and `ControlKey`s are valid like Tokenizer output,
  while restricting to only `Digit` in contexts like `DigitMapping` or `DigitAndCount`. We could have used a common enum
  and runtime validation to ensure ControlKeys don't leak into places where they don't belong, but the current implementation is simple enough and type safe.
...
- Organize into more projects ?

  Could have broken it down into a core project etc but deemed unnecessary given the scope of the problem.
---

# Assumptions

For many ambiguous cases, we have a source of truth.
Since we are simulating a real device... we can refer to them for behavior not provided in the example.
